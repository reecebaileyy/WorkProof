...
export default function SkillPetMint({
  contractAddress,
  onMintComplete,
}: SkillPetMintProps) {
  ...
  const { isLoading: isMintingConfirming, isSuccess: isMintSuccess } =
    useWaitForTransactionReceipt({
      hash: mintHash,
    });
  ...
  const registerResumeWallet = async (wallet: string) => {
    ...
    try {
      ...
      if (
        ...
        shortMessage.includes("already registered")
      ) {
        setError(null);
        ...
        const finalError =
          err instanceof Error
            ? err.message
            : "Failed to register resume wallet";
        setError(finalError);
      }
      setIsRegistering(false);
    }
  ...
  const mintSkillPet = async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to mint SkillPet";
      setError(errorMessage);
    }
  };
    ...
    return (
      <div className={styles.container}>
        <Card className={styles.success}>
          ...
          <p>
            Your SkillPet is ready. Start completing daily tasks to level up!
          </p>
        </Card>
      </div>
    );
        ...
        >
          ...
          {switchChainAsync && (
            <button
              ...
              style={{ marginTop: "1rem", background: "var(--warning)" }}
            >
  ...
  return (
    <div className={styles.container}>
      <Card className={styles.content}>
        <h2 className={styles.title}>Mint Your SkillPet NFT</h2>
        <p className={styles.description}>
          Your SkillPet NFT will track your skills and evolve as you complete
          daily tasks and earn attestations.
        </p>
        ...
        (resumeWallet &&
          ...
          <div className={styles.walletSetup}>
            <p>
              First, we need to create your resume wallet (Base Account
              sub-account)
            </p>
            {registeredWallet &&
            registeredWallet !==
              ...
              <div className={styles.walletInfo}>
                <p>
                  <strong>Resume Wallet:</strong> {registeredWallet.slice(0, 6)}
                  ...{registeredWallet.slice(-4)}
                </p>
                <p className={styles.info}>
                  Your resume wallet is already registered!
                </p>
              </div>
            ) : (
              <button
          ...
          <div className={styles.mintSection}>
            <div className={styles.walletInfo}>
              <p>
                <strong>Resume Wallet:</strong> {resumeWallet.slice(0, 6)}...
                {resumeWallet.slice(-4)}
              </p>
            </div>
            <button
        ...
        {(registerHash || mintHash) && (
          <p className={styles.txHash}>
            ...
            >
              {(registerHash || mintHash)?.slice(0, 10)}...
              {(registerHash || mintHash)?.slice(-8)}
            </a>
          </p>
        )}
      </Card>
